...
fn test_collection_construction() {
    ...
    let collection2 = Collection::new("users2", schema2.clone()).unwrap();
...
fn test_id_type_enforcement() {
    ...
    let mut string_collection = Collection::new("string_users", string_schema).unwrap();
    ...
    let mut int_collection = Collection::new("int_users", int_schema).unwrap();
