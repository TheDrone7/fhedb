...
fn parse_null() {
    let result = "null"
        .parse_as_bson(&FieldType::Nullable(Box::new(FieldType::Int)))
        .unwrap();
...
fn parse_boolean_true() {
    let result = "true".parse_as_bson(&FieldType::Boolean).unwrap();
    assert_eq!(result, Bson::Boolean(true));
}

#[test]
fn parse_boolean_false() {
    let result = "false".parse_as_bson(&FieldType::Boolean).unwrap();
    assert_eq!(result, Bson::Boolean(false));
}

#[test]
fn parse_int() {
    let result = "42".parse_as_bson(&FieldType::Int).unwrap();
...
#[test]
fn parse_float() {
    let result = "3.14".parse_as_bson(&FieldType::Float).unwrap();
...
fn parse_string_double_quotes() {
    let result = "\"hello world\"".parse_as_bson(&FieldType::String).unwrap();
...
fn parse_empty_string() {
    let result = "\"\"".parse_as_bson(&FieldType::String).unwrap();
...
fn parse_string_with_escape_sequences() {
    let result = "\"hello\\nworld\""
        .parse_as_bson(&FieldType::String)
        .unwrap();
...
fn parse_empty_array() {
    let result = "[]"
        .parse_as_bson(&FieldType::Array(Box::new(FieldType::Int)))
        .unwrap();
...
fn parse_array_of_floats() {
    let result = "[1.1, 2.2, 3.3]"
        .parse_as_bson(&FieldType::Array(Box::new(FieldType::Float)))
        .unwrap();
    assert_eq!(
        result,
        Bson::Array(vec![
...
fn parse_array_of_booleans() {
    let result = "[true, false, true]"
        .parse_as_bson(&FieldType::Array(Box::new(FieldType::Boolean)))
        .unwrap();
    assert_eq!(
        result,
        Bson::Array(vec![
...
fn parse_nested_arrays() {
    ...
    let result = "[[1, 2], [3, 4]]"
        .parse_as_bson(&FieldType::Array(Box::new(inner_type)))
        .unwrap();
    assert_eq!(
        result,
        Bson::Array(vec![
...
fn parse_reference_string() {
    let result = "\"ref-12345\""
        .parse_as_bson(&FieldType::Reference("other".to_string()))
        .unwrap();
    assert_eq!(result, Bson::String("ref-12345".to_string()));
}
...
fn parse_string_with_escaped_single_quote() {
    let result = "\"it\\'s\"".parse_as_bson(&FieldType::String).unwrap();
    assert_eq!(result, Bson::String("it's".to_string()));
}

#[test]
fn parse_id_int() {
    let result = "123".parse_as_bson(&FieldType::IdInt).unwrap();
    assert_eq!(result, Bson::Int64(123));
}

#[test]
fn parse_id_string() {
    let result = "\"user-abc\"".parse_as_bson(&FieldType::IdString).unwrap();
