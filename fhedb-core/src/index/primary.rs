//! # Primary Index
//!
//! Provides a B+ tree backed primary index that maps document IDs to data offsets.

use super::{pager::Pager, tree::BPlusTree};
use crate::document::DocId;
use std::{io, path::PathBuf};

/// A B+ tree backed primary index for a collection.
#[derive(Debug)]
...
impl PrimaryIndex {
    /// Creates a new [`PrimaryIndex`] for the specified field.
    ///
    /// ## Arguments
    ///
    /// * `field_name` - The name of the ID field being indexed.
    /// * `base_path` - The base directory path for collection storage.
    ///
    /// ## Returns
    ///
    /// Returns [`Ok`]\([`PrimaryIndex`]) if created successfully,
    /// or [`Err`]\([`io::Error`]) if the primary index could not be created.
